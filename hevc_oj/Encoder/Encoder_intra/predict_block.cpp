...
void getPredErrBlock(BUFFER* img, double* Pred_Err, uint8* Pred, uint8 p_size, uint32 ul_org)
{
	for (int j = 0; j < p_size; j++) {
		...
		{
			/* 원본 - 예측 = 예측에러버퍼(DCT 양자화x) */
			Pred_Err[j + i * p_size] = img->org_b[ul_org + j + i * img->nWidth] - Pred[j + i * p_size];
		}
	}
}
...
void getPredBlock(BUFFER* img, uint8* Pred, uint8 p_size, uint8 mode, uint32 ul_recon)
{
	...
	case 0:
		...
		for (j = 0; j < p_size; j++) {
			for (i = 0; i < p_size; i++)
			{
				Pred[j + i * p_size] = img->recon_padding[ul_recon + j + 1];
			}
		}
		break;
	...
	case 1:
		...
		for (j = 0; j < p_size; j++) {
			for (i = 0; i < p_size; i++)
			{
				Pred[j + i * p_size] = img->recon_padding[ul_recon + (i + 1) * (img->nWidth + 1)];
			}
		}
		break;
	...
	case 2:
	{
		...
		for (i = 0; i < p_size*p_size; i++) { Pred[i] = dc_tot; }
	...
	case 3:
	{
		...
		for (j = 0; j < p_size; j++) {
			for (i = 0; i < p_size; i++)
			{
				...
				Pred[j + i * p_size] = (i <= j) ? u_pxVal : l_pxVal;
			}
		}
		break;
	}
